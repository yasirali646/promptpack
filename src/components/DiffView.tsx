...
export default function DiffView({ original, compressed }: DiffViewProps) {
  ...
  if (removed.length === 0 && added.length === 0) {
    return <p className="diff-empty">Lines were restructured rather than removed outright.</p>;
  }
    ...
    <div className="diff">
      {removed.length > 0 && (
        <div className="diff__block">
          <h4 className="diff__heading">Struck</h4>
          <pre className="diff__pre diff__pre--removed">{removed.join("\n")}</pre>
        </div>
      )}
      {added.length > 0 && (
        <div className="diff__block">
          <h4 className="diff__heading">Set</h4>
